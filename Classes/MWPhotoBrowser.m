...
- (void)viewDidLoad {
    ...
    if (self.displayActionButton) {
        _actionButton = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"navi_more_icon"] style:UIBarButtonItemStylePlain target:self action:@selector(actionButtonPressed:)];
    }
...
- (void)setNavigationHidedn:(BOOL)hidden animated:(BOOL)animated permanent:(BOOL)permanent {
    [[UIApplication sharedApplication] setStatusBarHidden:hidden withAnimation:UIStatusBarAnimationSlide];
    if (hidden) {
        [UIView animateWithDuration:.3 animations:^{
            self.navigationController.navigationBar.frame = CGRectOffset(self.navigationController.navigationBar.frame, 0., -64.);
        }];
    } else {
        [UIView animateWithDuration:.3 animations:^{
            self.navigationController.navigationBar.frame = CGRectOffset(self.navigationController.navigationBar.frame, 0., 0.);
        }];
    }
}
...
//    
//    DLog(@"kkkkkkkkkkkkkkkkkkkkkkkkkkkk");
//    
//	if (![self areControlsHidden]) {
//        [self cancelControlHiding];
...
- (void)toggleControls {
    ...
    [self setNavigationHidedn:!isHidden animated:YES permanent:NO];
}
...
- (void)actionButtonPressed:(id)sender
{
    ...
    {
        _actionsSheet = [[UIActionSheet alloc]initWithTitle:nil delegate:self cancelButtonTitle:@"取消" destructiveButtonTitle:nil otherButtonTitles:@"保存图片", nil];
        _actionsSheet.tag = ACTION_SHEET_OLD_ACTIONS;
        _actionsSheet.actionSheetStyle = UIActionSheetStyleBlackTranslucent;
        _actionsSheet.cancelButtonIndex = _actionsSheet.numberOfButtons - 1;
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo {
    [self showProgressHUDCompleteMessage: error ? NSLocalizedString(@"Failed", @"Informing the user a process has failed") : NSLocalizedString(@"已保存到系统相册", @"Informing the user an item has been saved")];
//    [self hideControlsAfterDelay]; // Continue as normal...
}
...
- (void)copyPhoto {
    id <MWPhoto> photo = [self photoAtIndex:_currentPageIndex];
    if ([photo underlyingImage]) {
        [self showProgressHUDWithMessage:[NSString stringWithFormat:@"%@\u2026" , NSLocalizedString(@"拷贝中", @"Displayed with ellipsis as '拷贝中...' when an item is in the process of being copied")]];
